...
import sys, subprocess, traceback, csv, math,tkinter as tk, numpy as np, matplotlib.pyplot as plt
...
#YXdata = parse(filedialog.askopenfilename(title= "Select Horizontal Profile", filetypes=[('Comma Separated Values', '*.csv')]))
YXdata = parse("C:/Users/benne/OneDrive/Documents/GitHub/Misc-Python-Stuff/naca16021.csv")
#YZdata = parse(filedialog.askopenfilename(title= "Select Vertical Profile", filetypes=[('Comma Separated Values', '*.csv')]))
YZdata = parse("C:/Users/benne/OneDrive/Documents/GitHub/Misc-Python-Stuff/ys900.csv")

#Data preprocessing: Remove negative y values
YXdata = [item for item in YXdata if item[1] >= 0]
YZdata = [item for item in YZdata if item[1] >= 0]

#Data preprocessing: Sort list by ascending x values
YXdata = sorted(YXdata, key=lambda x: x[0])
YZdata = sorted(YZdata, key=lambda x: x[0])

#convert to numpy arrays
YXdata = np.unique(np.array(YXdata), axis=0)
YZdata = np.unique(np.array(YZdata), axis=0)
